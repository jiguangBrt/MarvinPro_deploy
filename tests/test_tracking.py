...
class TrackingGovernorTest(unittest.TestCase):
    def setUp(self):
        self.governor = TrackingGovernor(
            run_error_rad=0.01,
            resume_error_rad=0.03,
            stop_error_rad=0.04,
        )

    def test_scales_phase_rate_and_latches_hard_freeze(self):
        self.assertEqual(self.governor.update(0.005).phase_rate, 1.0)
        decision = self.governor.update(0.025)
        self.assertGreater(decision.phase_rate, 0.0)
        self.assertLess(decision.phase_rate, 1.0)

        frozen = self.governor.update(0.041)
        self.assertTrue(frozen.hard_frozen)
        self.assertEqual(frozen.phase_rate, 0.0)
        self.assertTrue(self.governor.update(0.035).hard_frozen)

        resumed = self.governor.update(0.029)
        self.assertFalse(resumed.hard_frozen)
        self.assertGreater(resumed.phase_rate, 0.0)

    def test_stale_state_and_clipping_freeze(self):
    ...
    def test_slow_first_order_robot_throttles_plan_phase(self):
        ...
        timeline = TrajectoryTimeline(knots, 7.5, 4)
        phase = 0.0
        measured = 0.0
        max_error = 0.0
        ...
        self.assertLessEqual(max_error, 0.0425)
